...
if TYPE_CHECKING:
    from .analyzer import FlopCounts, MemoryEstimates, MetricsEngine, ONNXGraphLoader, ParamCounts
...
class InspectionReport:
    ...
    def to_markdown(self) -> str:
        ...
        lines.append("")
        lines.append(f"*Generated by ONNX Autodoc v{self.autodoc_version} on {self.generated_at}*")
        lines.append("")
        ...
        lines.append(f"| Producer | {self.metadata.producer_name} {self.metadata.producer_version} |")
        ...
        if self.param_counts or self.flop_counts or self.memory_estimates:
            ...
            if self.param_counts:
                lines.append(f"- **Total Parameters**: {self._format_number(self.param_counts.total)}")
                lines.append(f"  - Trainable: {self._format_number(self.param_counts.trainable)}")
                lines.append(f"  - Non-trainable: {self._format_number(self.param_counts.non_trainable)}")
                lines.append("")

            if self.flop_counts:
                lines.append(f"- **Estimated FLOPs**: {self._format_number(self.flop_counts.total)}")
                lines.append("")

            if self.memory_estimates:
                lines.append(f"- **Model Size**: {self._format_bytes(self.memory_estimates.model_size_bytes)}")
                lines.append(
...
class ModelInspector:
    ...
    def inspect(self, model_path: str | pathlib.Path) -> InspectionReport:
        ...
        self.logger.info(f"Inspection complete. Found {len(detected_blocks)} blocks, {len(risk_signals)} risks.")
        return report

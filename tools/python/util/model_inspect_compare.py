...
"""
Compare-mode CLI for ONNX Autodoc.

Story 6.3: Compare Mode CLI
---------------------------

This CLI takes multiple ONNX model variants plus corresponding eval/perf
metrics JSON files and produces a single comparison JSON + Markdown
summary focused on quantization impact / multi-variant trade-offs.

It intentionally does NOT generate graph.html / PDF / HTML reports;
those remain the responsibility of the single-model `model_inspect` CLI.
Later stories (6.4, 6.10) can consume this compare JSON to render richer
visual dashboards.
"""
